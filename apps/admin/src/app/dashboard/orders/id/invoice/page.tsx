        ...
        <div className="bg-white min-h-screen">
            ...
            <div className="invoice-container border-4 border-black p-8 md:p-12 max-w-4xl mx-auto bg-white shadow-[20px_20px_0px_0px_rgba(0,0,0,0.05)]">
                ...
                <div className="flex flex-col md:flex-row justify-between gap-8 mb-12 border-b-4 border-black pb-8">
                    <div>
                        <h1 className="text-4xl font-black italic tracking-tighter uppercase leading-none mb-1">Fire Cutter</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
                    <div>
                        ...
                        <div className="space-y-1">
                            <p className="text-xs font-black uppercase">Fire Cutter Warehouse</p>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b-2 border-black">
                                <th className="text-left py-3 text-[10px] font-black uppercase tracking-widest">Description</th>
                                <th className="text-center py-3 text-[10px] font-black uppercase tracking-widest">Qty</th>
                                <th className="text-right py-3 text-[10px] font-black uppercase tracking-widest">Price</th>
                                <th className="text-right py-3 text-[10px] font-black uppercase tracking-widest">Total</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={idx}>
                                    <td className="py-4">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[9px] font-bold text-gray-400 font-mono uppercase">Variant: {item.variantSku?.split('-').pop()}</span>
                                        </div>
                                    </td>
                                    <td className="py-4 text-center text-xs font-black">{item.quantity}</td>
                                    <td className="py-4 text-right text-xs font-black">৳{item.price?.toLocaleString()}</td>
                                    <td className="py-4 text-right text-xs font-black">৳{(item.price * item.quantity).toLocaleString()}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                <div className="flex flex-col md:flex-row justify-between gap-8 pt-8 border-t-2 border-black">
                    <div className="max-w-xs">
                        ...
                        <p className="text-xs font-black italic uppercase mb-1">{order.paymentMethod === 'cod' ? 'Cash on Delivery' : 'Digital Transaction (bKash)'}</p>
                    ...
                    <div className="w-full md:w-64 space-y-3">
                        ...
                        <div className="flex justify-between items-end pt-3 border-t-2 border-black">
                            ...
                            <span className="text-2xl font-black italic leading-none">৳{order.totalAmount?.toLocaleString()}</span>
                        </div>
                    </div>
                </div>
