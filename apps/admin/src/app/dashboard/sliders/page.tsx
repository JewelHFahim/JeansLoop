    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center border-b-4 border-black pb-4">
                <div>
                    <h1 className="text-4xl font-black uppercase tracking-tighter italic">Hero Sliders</h1>
                    <p className="text-gray-500 text-sm font-bold tracking-widest uppercase mt-1">Manage Storefront Hero Carousel</p>
                </div>
                <Link href="/dashboard/sliders/new">
                    <Button className="h-12 border-2 border-black bg-black text-white px-6 font-bold uppercase tracking-widest text-xs hover:bg-white hover:text-black rounded-none transition-all shadow-[8px_8px_0_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-2 hover:translate-y-2">
            ...
            {isLoading ? (
                ...
                <div className="grid gap-6">
                    {sliders.length === 0 ? (
                        ...
                        sliders.map((slider) => (
                            <div key={slider.id} className="border-2 border-black bg-white p-4 flex gap-6 items-center shadow-[8px_8px_0_rgba(0,0,0,0.1)]">
                                <div className="w-48 h-32 flex-shrink-0 border-2 border-black relative overflow-hidden bg-gray-100">
                                    <img src={slider.image} alt="Slider image" className="object-cover w-full h-full" />
                                </div>
                                <div className="flex-1 space-y-2">
                                    <div className="flex items-center gap-3">
                                        <h3 className="font-black text-xl uppercase italic whitespace-pre-line">{slider.title}</h3>
                                        {!slider.isActive && (
                                    ...
                                    <p className="text-sm text-gray-600 font-medium line-clamp-2">{slider.subtitle}</p>
                                </div>
