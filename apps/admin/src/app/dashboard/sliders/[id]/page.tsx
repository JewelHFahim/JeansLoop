...
export default function EditSliderPage() {
    ...
    const { data: sliderResponse, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await slidersApi.getById(id);
            return res.data;
        },
    });
    ...
    useEffect(() => {
        if (sliderResponse?.data) {
            const s = sliderResponse.data;
            setFormData({
                title: s.title || '',
                subtitle: s.subtitle || '',
                image: s.image || '',
                primaryCtaText: s.primaryCta?.text || '',
                primaryCtaHref: s.primaryCta?.href || '',
                secondaryCtaText: s.secondaryCta?.text || '',
                secondaryCtaHref: s.secondaryCta?.href || '',
                order: s.order || 0,
    ...
    const updateMutation = useMutation({
        mutationFn: async (data: any) => {
            ...
            await slidersApi.update(id, payload);
        },
        onSuccess: () => {
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6 pb-20">
            <div className="flex items-center gap-4 border-b-4 border-black pb-4">
                ...
                <div>
                    <h1 className="text-4xl font-black uppercase tracking-tighter italic">Edit Slider</h1>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 bg-white border-2 border-black p-8 shadow-[8px_8px_0_rgba(0,0,0,0.1)]">
                ...
                <Button
                    type="submit"
                    disabled={updateMutation.isPending || isUploading || !formData.image}
                ...
                >
                    {updateMutation.isPending ? 'Updating...' : 'Update Slider'}
                </Button>
            </form>
        </div>
    );
